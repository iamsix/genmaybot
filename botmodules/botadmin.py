...
def __init__(self):
    ...
    if not result: #if table is not found
        sqlcur.execute('''create table admin_info(nick text, pm_monitor_enabled integer, PRIMARY KEY(nick))''')
        conn.commit()
    else:
...
def monitor_pm(line,nick,self,c):
    ...
    if not result: #if table is not found
        sqlcur.execute('''create table admin_info(nick text PRIMARY KEY, pm_monitor_enabled integer, PRIMARY KEY(nick))''')
        conn.commit()
