...
class ServerConnection(Connection):
    ...
    def process_data(self):
        ...
        try:
            if self.ssl:
                ...
                if isinstance(new_data,bytes):
                  try:
                    new_data = bytes.decode(new_data,'utf-8',)
                  except UnicodeDecodeError:
                    new_data = new_data.decode('Latin-1').encode('utf-8').decode('utf-8')
                  except:
                    new_data = "" #just give up
        except socket.error as x:
            # The server hung up.
        ...
        if isinstance(self.previous_buffer,bytes):
            try:
              self.previous_buffer = bytes.decode(self.previous_buffer,'utf-8')
            except UnicodeDecodeError:
              new_data = new_data.decode('Latin-1').encode('utf-8').decode('utf-8')
            except:
              new_data = "" #just give up
...
numevents = []
for ev in numeric_events.values():
	numevents.append(ev)


all_events = generated_events + protocol_events + numevents
